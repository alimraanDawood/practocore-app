...
function startPolling() {
  ...
  pollTimer = setInterval(() => {
    if (signedIn.value && sources.value.some((s) => s.status === 'pending' || s.status === 'processing')) {
      refresh();
    }
  }, 4000);
}
...
onMounted(() => {
  if (signedIn.value) { refresh(); startPolling(); }
});
</script>

<template>
  <div class="mx-auto min-h-dvh w-full max-w-6xl overflow-y-auto p-4 md:p-8">
    <!-- Header -->
