...
const getInviteStatus = (invite: any) => {
  ...
  if (invite.expiresAt) {
    if (dayjs().isAfter(dayjs(invite.expiresAt))) return 'expired';
  }
  return 'pending';
};
...
const getStatusBadge = (invite: any) => {
  ...
  switch (status) {
    case 'accepted':
      return { icon: CheckCircle, label: 'Accepted', class: 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' };
    case 'rejected':
      return { icon: XCircle, label: 'Rejected', class: 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' };
    case 'expired':
      return { icon: Clock, label: 'Expired', class: 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300' };
    default:
      return { icon: Clock, label: 'Pending', class: 'bg-orange-100 text-orange-700 dark:bg-orange-900 dark:text-orange-300' };
  }
};
...
const filteredInvites = computed(() => {
  ...
  if (props.searchQuery?.trim()) {
    ...
    filtered = filtered.filter((invite: any) =>
  ...
  if (props.statusFilter && props.statusFilter !== 'all') {
    filtered = filtered.filter((invite: any) => getInviteStatus(invite) === props.statusFilter);
  }
...
const inviteStats = computed(() => {
  ...
  return {
    pending: items.filter((i: any) => getInviteStatus(i) === 'pending').length,
    accepted: items.filter((i: any) => getInviteStatus(i) === 'accepted').length,
    expired: items.filter((i: any) => getInviteStatus(i) === 'expired').length,
  };
});
...
// Expose result count so parent can show it in the search bar
defineExpose({ count: computed(() => filteredInvites.value.length) });
</script>
...
<template>
  <div class="flex flex-col gap-4 p-3 h-full overflow-y-scroll">
    ...
    <!-- Invitations List -->
    <div v-else class="flex flex-col xs:grid xs:grid-cols-2 md:grid-cols-3 gap-3 h-full overflow-y-scroll items-stretch content-start">
      <div
        ...
        <div class="flex flex-row gap-2 shrink-0">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem
                v-if="getInviteStatus(invite) === 'pending' || getInviteStatus(invite) === 'expired'"
    ...
    <div class="text-sm text-muted-foreground text-center pt-1">
