    ...
    <div class="flex flex-col w-full gap-1">
      ...
      <div class="grid grid-cols-7 border rounded-md divide-x divide-y">
        <div v-for="(cell, idx) in cells" :key="idx"
          ...
          <div class="hidden flex-col gap-1 mt-1 w-full">
            <template v-for="(evt, i) in visibleEvents(cell.date)" :key="evt.id">
              <div class="text-[11px] px-1.5 py-0.5 rounded cursor-pointer truncate border"
                    :class="`bg-${evt.color}/50 text-${evt.color} border-${evt.color}`"
                @click.stop="eventClicked(cell.date, evt.id)">
                {{ evt.title }}
              </div>
            </template>
          </div>

          <div class="flex flex-row gap-1 mt-1 w-full items-center justify-center">
            <template v-for="(evt, i) in visibleEvents(cell.date)" :key="evt.id">
              <div class="text-[11px] size-[6px] rounded-full cursor-pointer truncate border"
                ...
                :style="{ backgroundColor: evt.color ? evt.color + '' : undefined, borderColor: evt.color || undefined, color: evt.color ? undefined : 'inherit' }">
              </div>
            </template>
          </div>
        </div>
...
const eventClicked = (date : Date, id : string) => {
  currentDate.value = date;
  emits('event-click', id);
}
